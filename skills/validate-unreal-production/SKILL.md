...
Validate Stage 2 as one versioned Area Composition Plan package, not as a mandatory file count. Floor-plan/footprint, node/connectivity, annotated top-down, and any conditionally required side/elevation components may be integrated or separately linked when each remains identifiable and reviewable. Once the plan-view evidence is inspectable and before profile production or package-scope finalization, require a `vertical_information_assessment` covering terrain relief, multi-level circulation, cliffs/overhangs/tunnels/bridges, sightline occlusion, and architecture-terrain vertical relationships, plus evidence that its result and basis were communicated to the user or designated approver. Accept `INSUFFICIENT` only when the named side/elevation profiles are present and scoped; accept `SUFFICIENT` only with an omission reason and an alternative-evidence mapping for every applicable risk; reject `PENDING_EVIDENCE` or an unrecorded decision. This assessment does not satisfy Chapter 04's Terrain Representation Review or select Landscape versus Mesh Terrain.
