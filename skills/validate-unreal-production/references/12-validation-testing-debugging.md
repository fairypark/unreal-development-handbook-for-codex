...
| Area Composition Plan | Before the first cube or broad asset placement, a versioned package exposes floor-plan/footprint, node/connectivity, and annotated top-down functions without requiring separate files for each label. Once the plan-view evidence is inspectable and before profile production or package-scope finalization, a `vertical_information_assessment` tests top-down sufficiency for terrain relief, multi-level circulation, cliffs/overhangs/tunnels/bridges, sightline occlusion, and architecture-terrain vertical relationships, and records communication of the result and basis to the user or designated approver. `INSUFFICIENT` requires the named side/elevation profiles; `SUFFICIENT` requires an omission reason and alternative evidence for every applicable risk. The package also assigns zone boundaries and stable IDs, terrain elevations and steps, primary circulation, rivers and bridges, building footprints and typology hierarchy, and a stable-ID `DIAGNOSTIC_ONLY` overview-camera set. Its coverage matrix closes arrival, reverse, lateral, waterway/axis, elevation, and project-specific spatial risks with the smallest sufficient set, and records auxiliary player-height/FOV proxies plus the planned runtime-rig source. | Missing or retrospective package; mandatory file-count substituted for reviewable functions; absent or `PENDING_EVIDENCE` vertical assessment; missing required profile; unsupported profile omission; missing pre-profile-production communication; unreadable route, elevation, water crossing, footprint, camera authority, or coverage; reliance on one hero overview or an arbitrary camera count; typology approved by size alone; or an unowned critical dependency. |
...
For Stage 2, bind every vertical-risk finding to its controlling source, review question, measurement basis, evidence IDs, and either the required profile component or the recorded alternative evidence. Preserve the pre-production communication record and the plan-package component inventory. A profile's presence alone does not prove sufficiency, and an attractive top-down does not prove that an omitted profile was unnecessary.
...
- Automatically producing every side/elevation profile, requiring one file per plan-view label, or omitting profiles under vague discretion instead of recording top-down sufficiency, pre-production user communication, and risk-by-risk alternative evidence.
