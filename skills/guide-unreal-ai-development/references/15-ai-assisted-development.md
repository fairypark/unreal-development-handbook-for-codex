...
A Stage 2 Area Composition Plan uses a communication-bearing evidence decision, not automatic profile drawing. Once its plan-view functions are inspectable and before producing side/elevation profiles or finalizing package scope, record a `vertical_information_assessment` that tests whether top-down evidence can verify terrain relief, multi-level circulation, cliffs/overhangs/tunnels/bridges, sightline occlusion, and architecture-terrain vertical relationships. Explain the result and basis to the user or designated approver before profile production. `INSUFFICIENT` adds named, scoped side/elevation profiles to the required package; `SUFFICIENT` records why they add no material value and maps each applicable risk to alternative evidence; `PENDING_EVIDENCE` stops profile production, package finalization, and promotion while allowing only the plan-view refinement or evidence gathering needed to resolve the decision. The plan is one versioned review package whose floor-plan/footprint, node/connectivity, annotated top-down, and conditional profile functions may be integrated or separately linked; no rule requires one file per label. Keep this decision separate from the Stage 2 feedback verdict, Stage 2a placement lock, and the Terrain Representation Review.
...
- For Stage 2, the workflow records a communicated `vertical_information_assessment`; every `INSUFFICIENT` risk has a named profile component, every `SUFFICIENT` omission has risk-by-risk alternative evidence, and `PENDING_EVIDENCE` prevents plan production or promotion without imposing a separate-file count.
...
- Automatically producing side/elevation profiles, silently omitting them under vague discretion, or forcing floor plan, node map, top-down, side, and elevation functions into separate files without a Stage 2 risk-and-evidence decision communicated before production.
