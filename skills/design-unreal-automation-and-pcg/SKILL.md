...
- When a graph both reads terrain or partition data and writes a result back into that world, apply Chapter 09's **Bidirectional terrain and PCG data flow** contract: immutable source revision, separate query/write layers, explicit priority or sub-priority, feedback-loop protection, and repeated-generation, persistence, runtime, performance, and rollback evidence.
